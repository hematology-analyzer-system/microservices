...
def run_docker_prune():
    ...
    if not confirm_action("Are you sure you want to proceed with a full Docker system prune (including volumes and stopping all containers)?"):
    ...
    try:
        ...
        subprocess.run(["docker", "system", "prune", "--all", "--volumes", "--force"], check=True)
